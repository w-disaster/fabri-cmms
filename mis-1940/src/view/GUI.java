...
import controllers.DBMSController;
import controllers.DBMSControllerImpl;
import controllers.ItemController;
import controllers.ItemControllerImpl;
import controllers.MaintenanceScheduleController;
import controllers.MaintenanceScheduleControllerImpl;
import controllers.ProjectController;
import controllers.ProjectControllerImpl;
import controllers.SparePartController;
import controllers.SparePartControllerImpl;
import controllers.WorkDescriptionController;
import controllers.WorkDescriptionControllerImpl;
import controllers.WorkOrderController;
import controllers.WorkOrderControllerImpl;
...
public class GUI extends JFrame {
	...
	/** The spare part logics. */
	private SparePartController sparePartController;
	
	/** The work description logics. */
	private WorkDescriptionController workDescriptionController;
	
	/** The ms logics. */
	private MaintenanceScheduleController msController;
	
	/** The project logics. */
	private ProjectController projectController;
	
	/** The work order logics. */
	private WorkOrderController workOrderController;
	
	/** The project WO table. */
	...
	public GUI() {
		...
		// CONTROLLERS
		this.dbmsController = new DBMSControllerImpl();
		this.itemController = new ItemControllerImpl(this.dbmsController);
		this.sparePartController = new SparePartControllerImpl(this.dbmsController);
		this.workDescriptionController = new WorkDescriptionControllerImpl(this.dbmsController);
		this.msController = new MaintenanceScheduleControllerImpl(this.dbmsController);
		this.projectController = new ProjectControllerImpl(this.dbmsController);
		this.workOrderController = new WorkOrderControllerImpl(this.dbmsController);
		...
		JComboBox<String> cmbItemUnit = new JComboBox<>(units);
		...
		this.itemController.buildAttributes(txtItemSerialNO, txtItemName, txtItemModel, txtItemMaker, txtItemDimension, txtItemYear,
				cmbItemUnit, txtItemADR);
		
		JComboBox<String> cmbItemFamily = new JComboBox<>(new String[] {"Plastic product manufacturing", "Automotive manufacturing", "Refineries",
		...
		JComboBox<String> cmbItemSubfamily = new JComboBox<>();
		cmbItemSubfamily.setBounds(146, 266, 124, 19);
		itemRegistrationPanel.add(cmbItemSubfamily);
		
		
		JComboBox<String> cmbItemCity = new JComboBox<>(new String[] {"Roma", "Rimini", "Milano", "Bologna", "Torino",
				"Ancona", "Senigallia"});
		cmbItemCity.setBounds(146, 295, 124, 19);
		itemRegistrationPanel.add(cmbItemCity);
		
		JComboBox<String> cmbItemAddress = new JComboBox<>();
		...
		JComboBox<String> cmbItemCodSupplier = new JComboBox<>(new String[] {"ULIVIERI ricambi srl", "DMG MORI"});
		cmbItemCodSupplier.setBounds(146, 384, 124, 19);
		itemRegistrationPanel.add(cmbItemCodSupplier);
		
		//LOGICS: BUILD FOREIGN KEYS
		this.itemController.buildForeignKeys(cmbItemFamily, cmbItemSubfamily, cmbItemCity, cmbItemAddress, cmbItemCodSupplier);
		...
		this.itemController.buildOptionals(txtMCDateConclusion, txtMCExpiration, txtNumWarranty, 
		...
		btnChooseImage.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if(input == JFileChooser.APPROVE_OPTION) {
					...
					itemController.uploadImage(f.getAbsolutePath());
					setItemImage(f);
				}
		...
		btnChooseFile.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if(input == JFileChooser.APPROVE_OPTION) {
					itemController.uploadFile(jfc.getSelectedFile().getAbsolutePath());
				}
				super.mouseClicked(e);
			}
		...
		this.itemController.buildImTxtTable(txtAreaDescriptions, txtAreaSpecifications,  dtmItem);
		...
		JComboBox<String> cmbSPCodSupplier = new JComboBox<>(new String[]{"ULIVIERI ricambi srl", "DMG MORI"});
		...
		JComboBox<String> cmbCodWarehouse = new JComboBox<>(new String[]{"1", "2", "3"});
		...
		btnRegisterPurchase.addMouseListener(new MouseAdapter() {
			...
			@Override
			public void mouseClicked(MouseEvent e) {
				sparePartController.savePurchase();
				super.mouseClicked(e);
			}
		...
		JComboBox<String> cmbSparePart = new JComboBox<>(new String[]{"ricambio 1", "ricambio 2", "pezzo x", "pezzo y", "pezzo z"});
		cmbSparePart.setBounds(355, 43, 124, 19);
		stockPanel.add(cmbSparePart);
		
		this.sparePartController.buildStock(cmbSparePart, txtStockAmount, txtSPUnitCost, txtStockNotesArea, cmbCodWarehouse);
		...
		this.sparePartController.buildPurchase(cmbSPCodSupplier, txtSPBillNumber, txtSPPurchaseDate, dtmSparePart);
		...
		this.workDescriptionController.buildWorkDescription(txtWDSerialNumber, txtWDDescription, WDTasksTxtArea, WDNotesTxtArea, dtm);
		...
		btnWDAddStock.addActionListener(l -> {
			workDescriptionController.saveUses();
		});
		btnWDAddStock.setBounds(223, 178, 114, 25);
		WDStockPanel.add(btnWDAddStock);
		
		JComboBox<String> cmbWDSupplier = new JComboBox<>(new String[]{"ULIVIERI ricambi srl", "DMG MORI"});
		cmbWDSupplier.setBounds(147, 86, 135, 19);
		WDStockPanel.add(cmbWDSupplier);
		
		JComboBox<String> cmbWDSparePart = new JComboBox<>(new String[]{"ricambio 1", "ricambio 2", "pezzo x", "pezzo y", "pezzo z"});
		cmbWDSparePart.setBounds(147, 36, 135, 19);
		WDStockPanel.add(cmbWDSparePart);
		
		this.workDescriptionController.buildUses(cmbWDSupplier, txtWDBillNumber, cmbWDSparePart, txtWDStockAmount);
		...
		btnAddManHours.addActionListener(l -> {
			workDescriptionController.saveManHours();
		});
		btnAddManHours.setBounds(192, 178, 151, 25);
		manHoursPanel.add(btnAddManHours);
		
		JComboBox<String> cmbWDCF = new JComboBox<>(new String[]{"FBRLCU99R19I608E"});
		...
		btnAddWorkDescription.addActionListener(l -> {
			workDescriptionController.saveWorkDescription();
		});
		...
		btnClearAll_1.addActionListener(l -> {
			this.workDescriptionController.clearAll();
		});
		...
		this.workDescriptionController.buildManHours(cmbWDCF, txtMHHours, txtMHCostHour);
		//SCHEDULE TAB
		...
		JComboBox<String> cmbWorkType = new JComboBox<>(workTypes);
		...
		btnRegisterMS.addActionListener(l -> {
			msController.saveMaintenanceSchedule();
		});
		btnRegisterMS.setBounds(414, 335, 250, 25);
		maintenancePanel.add(btnRegisterMS);
		
		this.msController.build(txtMSDescription, txtNumMS, txtMSSerialNO, cmbWorkType, workTypeTxtArea, 
		...
		JComboBox<String> cmbProjectWorkType = new JComboBox<>(workTypes);
		...
		btnFilterWorkOrders.addActionListener(l -> {
			projectController.filterWorkOrders();
		});
		...
		this.projectController.build(txtProjectDescription, txtCodProject, cmbProjectWorkType, projectWorkTypeTxtArea, 
		...
		JComboBox<String> cmbUrgencyLevel = new JComboBox<>(urgencyLevels);
		...
		this.workOrderController.buildWorkOrder(txtNumWO, txtWOSerialNO, txtWODescription, txtPlannedTimeMaintenance,
		...
		btnAddWOWD.addActionListener(l -> {
			workOrderController.saveExecute();
		});
		...
		JComboBox<String> cmbWOTrigger = new JComboBox<>(extraWOTriggers);
		...
		JComboBox<String> cmbWOType = new JComboBox<>(new String[]{"Ordinary", "Extraordinary"});
		...
		this.workOrderController.buildWorkOrderType(cmbWOType, textWONumMS, txtWOCodProject , cmbWOTrigger, 
	 ...
	 * Sets the item image.
	 *
	 * @param imageFile the new item image
	 */
